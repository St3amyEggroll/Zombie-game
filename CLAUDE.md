...
| Launch pass | **Shipped Sep 2026 (pre-launch):** `TelemetryService` (game) + `LAUNCH` table (lobby) log Roblox **AnalyticsService** funnels (onboarding ladder shared across both places, a per-run "Run" funnel, per-wave Coin economy events, `run_end`); **badges** (`GameConfig.BadgeIds` / `LAUNCH.Badges`, ids = 0 until the owner creates them); **friend bonus** (`GameConfig.FriendCoinMult` — a Roblox friend in the server = ×1.25 Coins, HUD chip); **paid-random-items policy** (PolicyService: restricted players lose the paid re-spin / starter crates / VIP crate — paid in Coins instead, prompts hidden via `ShopSync.restricted`); **touch controls** (`TouchControlsController`: FIRE / RUN / USE, Autofire defaults ON on phones, raw touches no longer fire); **run recap** card + like-nudge and the **INVITE** circle + `UpdateBoard`-tagged WHAT'S NEW sign (`LobbyExtras.client.lua` — a separate script because `LobbyClient` sits at the 200-local ceiling). Perf: `MaxAliveZombies` 80, cached raycast params, no ragdoll for fast-pooled corpses, spawn ring no longer stalls. |
